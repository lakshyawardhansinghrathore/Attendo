...
def teacher_tab_manage_subjects():
    ...
    if subjects:
        for idx, sub in enumerate(subjects):
            ...
            def share_btn(s_name=sub.get('name', ''), s_code=sub.get('subject_code', ''), i=idx):
                if st.button(f"Share Code: {s_name}", key=f"share_{s_code}_{i}", icon=":material/share:"):
...
def teacher_tab_analytics():
    ...
    if not records:
        st.info("No attendance data available to show analytics.")
        return
    ...
    for r in records:
        ...
        student_name = r.get('students', {}).get('name', 'Unknown Student') if r.get('students') else 'Unknown Student'
    ...
    trend_df = df[df['is_present'] == True].groupby('Date').size().reset_index(name='Present Count')
    if not trend_df.empty:
        trend_df = trend_df.set_index('Date')
        st.line_chart(trend_df)
    else:
        st.info("No 'Present' records found yet to show a trend.")
    ...
    student_stats['Attendance %'] = (student_stats['Classes_Attended'] / student_stats['Total_Classes']) * 100
    
    # Filter for low attendance (< 75%)
    low_attendance = student_stats[student_stats['Attendance %'] < 75].sort_values('Attendance %')
    
    if not low_attendance.empty:
        st.warning(f"Found {len(low_attendance)} students with attendance below 75%!")
        display_low = low_attendance[['Student', 'Attendance %', 'Total_Classes']].copy()
        display_low['Attendance %'] = display_low['Attendance %'].round(1).astype(str) + "%"
        st.dataframe(display_low, width='stretch', hide_index=True)
    else:
        st.success("Great job! All students have an attendance of 75% or higher.")
